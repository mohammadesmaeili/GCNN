...
def fun_simulator_new(dataset, side_generator, k=None, alpha=None, classifier=None, radius=None, side=None, seed=None, Eu=None, epochs=None, beta=None):
    np.random.seed(seed)
    ...
    if side_generator =='extracted':
        ...
        z_cat = tf.keras.utils.to_categorical(z, num_classes=initial['classes'])
        if dataset=='k-sbm':
            X = z_cat
        else:
            X = np.concatenate([X, z_cat], axis=1)
        initial['features'] = X.shape[1]
        temp = initial['layers']
        temp[0] = initial['features']
        initial['layers'] = temp
        #initial['Eu'] = initial['epochs']
